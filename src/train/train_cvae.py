...
def do_epochs(model, datasets, parameters, optimizer, writer):
    ...
    if parameters['beta_sigmoid']:
        print('**** Train with beta_sigmoid ****')
        model.sigmoid_weight(parameters['beta_max'], parameters['beta_sigmoid_weight'], parameters['beta_sigmoid_bias'])
    if parameters['beta_cycle']:
        print('**** Train with beta_cycle ****')
        model.cycle_weight(len(train_iterator)*parameters['num_epoch'], stop=parameters['beta_max'], n_cycle=parameters['beta_cycle_n'], ratio=parameters['beta_cycle_ratio'])

    logpath = os.path.join(parameters["folder"], "training.log")
    with open(logpath, "w") as logfile:
        for epoch in range(1, parameters["num_epochs"]+1):
            dict_loss = train(model, optimizer, train_iterator, model.device, parameters, epoch)

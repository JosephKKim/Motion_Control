...
class MotionTrans():
    def __init__(self, prob=0.3):
        self.prob = prob
        # self.leg_idx = [1, 2, 4, 5, 7, 8, 10, 11]
    def __call__(self, pose, cls):
        # pose list of pose with size [60, 25, 6]
        if random.random() <= self.prob:
        
            mat = geometry.rotation_6d_to_matrix(pose)

            mat_np = mat.numpy().squeeze()
            angles = []
            for i in range(mat_np.shape[1]):
                angle = R.from_matrix(mat_np[:, i, :, :])
                a = angle.as_euler('zyx', degrees = True)
                angles.append(a)


            stacked_angle = np.stack(angles, axis = 0)
            # unit of diff is degrees!

            diff = np.diff(stacked_angle, axis=1)
            # diff shape [25, 59, 3]
            # selected_diff = diff[self.leg_idx, :, :]

            # uniform variable between 1 and 1.5
            alpha = 0.5 * np.random.rand(1) + 1
            # alpha = 1.5
            # print(alpha)
            # selected_diff *= alpha
            
            # apply to leg only 
            # diff[self.leg_idx] = diff[self.leg_idx] * alpha
        

            # apply to all
            diff[1:-1, :, :] = diff[1:-1, :, :] * alpha
            n_stacked_angle = np.concatenate((stacked_angle[:,0:1,:], diff), axis=1).cumsum(axis = 1)

            
            # Inverse procedure
            matrices = []
            for i in range(n_stacked_angle.shape[0]):
                mat_from_a = R.from_euler('zyx', n_stacked_angle[i, :, :], degrees=True).as_matrix()
                mat_from_a = torch.from_numpy(mat_from_a)
                # print(mat_from_a)
                matrices.append(mat_from_a)

            matrices_torch = torch.stack(matrices, dim = 1)
            final_6d = geometry.matrix_to_rotation_6d(matrices_torch)
            # shape [60, 25, 6] -> [25, 6, 60]
            final_6d = final_6d.permute(1,2,0).contiguous().type(torch.float32)
            return final_6d, cls
        else: 
            pose = pose.permute(1,2,0).contiguous().type(torch.float32)
            return pose, cls
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=1, sampling="conseq", sampling_step=1, split="train",
        ...
        self.transform = None if kwargs['data_aug'] == 1.0 else MotionTrans(kwargs['data_aug'])
        ...
        if kwargs['data_aug'] > 1.0 or kwargs['data_aug'] < 0:
            raise ValueError("argument for data_aug should be between 0 and 1")

        super().__init__()

        print(self.transform)
        if self.transform is not None:
            print(self.transform.prob)
        # to remove shuffling
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            inp = inp.permute(2,0,1).contiguous()

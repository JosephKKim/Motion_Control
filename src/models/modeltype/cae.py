...
class CAE(nn.Module):
    def __init__(self, encoder, decoder, device, lambdas, latent_dim, outputxyz,
                 pose_rep, glob, glob_rot, translation, jointstype, vertstrans, beta_sigmoid, beta_cycle, **kwargs):
        super().__init__()
    ...
    def sigmoid_weight(self, max_val, weight, bias):
        self.sigmoid_weight_fn = lambda x : (1/(1+math.exp(weight*x+bias))) * max_val

    def cycle_weight(self, n_iter, start=0.0, stop=1.0,  n_cycle=4, ratio=0.5):
        ...
        for ltype, lam in self.lambdas.items():
            ...
            if ltype == 'kl':
                if self.beta_sigmoid:
                    mixed_loss += loss*self.sigmoid_weight_fn(step)
                elif self.beta_cycle:
                    mixed_loss += loss*self.cycle_weight_list[step]
                else:
                    mixed_loss += loss*lam
            losses[ltype] = loss.item()
        losses["mixed"] = mixed_loss.item()

        # to check beta_weight
        if self.beta_sigmoid:
            losses["beta_weight"] = self.sigmoid_weight_fn(step)
        return mixed_loss, losses

    def compute_loss_beta_cycle(self, batch, weight=None):
        mixed_loss = 0
        losses = {}
        for ltype, lam in self.lambdas.items():
            loss_function = get_loss_function(ltype)
